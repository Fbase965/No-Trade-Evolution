import struct
with open("E:/Pokemon-XY-No-Trade-Mod/tools/check_a216.py", "r") as f:
    pass
...
data_start = fimg_off + struct.unpack('<I', garc_evo[fimg_off+4:fimg_off+8])[0] # usually fimg header is 8 or 12 bytes

print(f"data_start: {hex(data_start)}, fimg_off: {hex(fimg_off)}")

for idx in [1, 2, 64, 93, 123, 133]:
    e_off = fatb_off + 12 + idx * 16
    flags, start, end, length = struct.unpack('<IIII', garc_evo[e_off:e_off+16])
    raw = garc_evo[data_start + start : data_start + end]
    print(f"Species {idx}: flags={flags}, start={start}, end={end}, len={length}, raw={raw.hex(' ')}")
    # Parse evolution slots (each slot is 6 bytes: method u16, param u16, target u16, or 8 bytes)
    for s in range(0, len(raw), 6):
        if s + 6 <= len(raw):
            method, param, target = struct.unpack('<HHH', raw[s:s+6])
            if method != 0 or target != 0:
                print(f"   Slot {s//6}: Method={method}, Param={param}, TargetSpecies={target}")
